...
df -h |grep "scratch"
echo " "
total_submittied=$(grep "Submitted" $LOGFILE|wc -l)
...
echo "Number of Jobs in the queue ${queuing}, whith $running running"
echo "***********"
...
echo "*** Stage 4: BQSR"
successful=$(grep "BQSR" $LOGFILE|grep "successful"|wc -l)
echo "Number of successful Jobs: $successful"
queuing=$(qstat|grep "BQSR"|wc -l)
running=$(qstat -s r|grep "BQSR"|wc -l)
echo "Number of Jobs in the queue ${queuing}, whith $running running"
echo "***********"
