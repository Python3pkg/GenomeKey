...
class AlignAndClean(bwa.MEM,picard.AddOrReplaceReadGroups,picard.CollectMultipleMetrics):
    ...
    def cmd(self,i,s,p):
        ...
        return r"""
            set -o pipefail &&  LD_PRELOAD=libhugetlbfs.so HUGETLB_MORECORE=yes HUGETLB_ELFMAP=RW {s[bwa_path]} mem -M -t 5
...
class Bam_To_FastQ(picard.REVERTSAM):
    ...
    def cmd(self,i,s,p):
        return r"""
            set -o pipefail && LD_PRELOAD=libhugetlbfs.so HUGETLB_MORECORE=yes HUGETLB_ELFMAP=RW {s[samtools_path]} view -h -u -r {p[rgid]} {i[bam][0]} {p[sn]}
            |
            {s[java]} -Xms1G -Xmx2G
            ...
            COMPRESSION_LEVEL=0
            |
             LD_PRELOAD=libhugetlbfs.so HUGETLB_MORECORE=yes HUGETLB_ELFMAP=RW {s[bamUtil_path]} bam2FastQ --in -.ubam
